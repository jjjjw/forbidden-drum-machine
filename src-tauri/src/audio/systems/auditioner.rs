...
impl AuditionerSystem {
    ...
    /// Trigger kick drum for testing
    pub fn trigger_kick(&mut self) -> Result<(), String> {
        self.kick.handle_event("trigger", 0.0)
    }
    
    /// Trigger clap drum for testing
    pub fn trigger_clap(&mut self) -> Result<(), String> {
        self.clap.handle_event("trigger", 0.0)
    }
}
...
impl AudioSystem for AuditionerSystem {
    fn handle_node_event(&mut self, node_name: &str, event_name: &str, parameter: f32) -> Result<(), String> {
        match node_name {
            ...
            "system" => {
                ...
                match event_name {
                    "set_delay_send" => {
                        self.set_delay_send(parameter);
                        Ok(())
                    }
                    "set_reverb_send" => {
                        self.set_reverb_send(parameter);
                        Ok(())
                    }
    ...
    fn set_sequence(&mut self, sequence_config: &serde_json::Value) -> Result<(), String> {
        ...
        println!("AuditionerSystem: set_sequence called with: {}", sequence_config);
        Ok(())
    }
