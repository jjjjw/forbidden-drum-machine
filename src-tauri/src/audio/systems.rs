...
use crate::audio::effects::{DelayLine, AllpassReverb};
...
impl DrumMachine {
    pub fn new() -> Self {
        ...
        Self {
            ...
            reverb_size_mod: SampleAndHold::new(0.165, 0.3, 0.8, 200.0), // 6 sec updates, 200ms slew  
    ...
    pub fn next_sample(&mut self) -> (f32, f32) {
        ...
        let delay_samples = (modulated_delay_time * SAMPLE_RATE) as usize;
        let delay_output = self.delay.process(delay_input, delay_samples, 0.4);
        let (reverb_l, reverb_r) = self.reverb.process(reverb_input, reverb_input);
        ...
        let output_l = dry_mixed * dry_level + (delay_output + reverb_l) * wet_level;
        let output_r = dry_mixed * dry_level + (delay_output + reverb_r) * wet_level;
    ...
    pub fn set_reverb_wet_mix(&mut self, wet: f32) {
        self.reverb.set_wet_mix(wet);
    }
    
    pub fn set_reverb_highpass(&mut self, freq: f32) {
        self.reverb.set_highpass_freq(freq);
    }
    
    pub fn set_reverb_lowpass(&mut self, freq: f32) {
        self.reverb.set_lowpass_freq(freq);
    }
...
mod tests {
    ...
    fn test_audio_output_one_bar() {
        ...
        println!("BPM: 120, Sample rate: {}, Samples per step: {:.0}, Total samples: {}", 
        ...
        assert!(max_amplitude < 1.0, "Audio clipping detected (amplitude >= 1.0)");
