...
impl AREnvelope {
    ...
    fn calculate_parameters(&mut self) {
        self.attack_samples = (self.attack_time * SAMPLE_RATE) as u32;
        self.release_samples = (self.release_time * SAMPLE_RATE) as u32;
    ...
    fn apply_curve(&self, progress: f32, curve_type: CurveType) -> f32 {
        match curve_type {
...
mod tests {
    ...
    fn test_ar_envelope_basic_operation() {
        ...
        while env.state == EnvelopeState::Attack {
            ...
            samples_in_attack += 1;
            if samples_in_attack > 10000 { // Safety break
                break;
            }
        }
        
        println!("Attack phase: {} samples, max level: {}", samples_in_attack, max_level);
        
        // Process through release phase  
        while env.is_active() {
            let _level = env.next_sample();
            samples_in_release += 1;
            if samples_in_release > 10000 { // Safety break
                break;
            }
        }
    ...
    fn test_ar_envelope_levels() {
        ...
        while env.is_active() {
            ...
            if all_levels.len() > 2000 { // Safety break
                break;
            }
        }
        ...
        println!("Envelope levels - min: {}, max: {}, total samples: {}", 
               min_level, max_level, all_levels.len());
        println!("First 10 levels: {:?}", &all_levels[..all_levels.len().min(10)]);
        println!("Last 10 levels: {:?}", &all_levels[all_levels.len().saturating_sub(10)..]);
    ...
    fn test_curve_types_preserve_timing_and_amplitude() {
        ...
        let curves = [CurveType::Linear, CurveType::Exponential, CurveType::Logarithmic];
        ...
        for &attack_curve in &curves {
            for &release_curve in &curves {
                ...
                while env.state == EnvelopeState::Attack {
                    ...
                    max_level = max_level.max(level);
                    samples_in_attack += 1;
                    if samples_in_attack > 5000 { break; }
                }
                ...
                while env.is_active() {
                    ...
                    levels.push(level);
                    samples_in_release += 1;
                    if samples_in_release > 10000 { break; }
                }
                ...
                assert!((samples_in_attack as i32 - expected_attack_samples as i32).abs() <= 1,
                    "Attack timing should be consistent for curve {:?}", attack_curve);
                assert!((samples_in_release as i32 - expected_release_samples as i32).abs() <= 1,
                    "Release timing should be consistent for curve {:?}", release_curve);
                
                // Maximum amplitude should always reach 1.0 regardless of curve
                assert!((max_level - 1.0f32).abs() < 0.001,
                    "Max level should be 1.0 for all curves, got {} with curve {:?}", max_level, attack_curve);
                
                // Envelope should end at 0
                assert!(!env.is_active(), "Envelope should be inactive after completion");
